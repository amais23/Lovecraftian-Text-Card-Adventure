...
export const BalanceMatrixDashboard: React.FC = () => {
  ...
  // ADR-0038: Deck Topology & Emergent Archetypes states
  const allDeckNodes = useMemo(() => balanceData.deckTopology || [], []);
  const emergentArchetypes = useMemo(() => balanceData.emergentArchetypes || [], []);
  ...
  const diffStats = useMemo(() => {
    if (!selectedDeckA || !selectedDeckB) return null;
    return {
      scoreDelta: selectedDeckA.overallScore - selectedDeckB.overallScore,
      cardsDelta: selectedDeckA.totalCards - selectedDeckB.totalCards,
      handDelta: selectedDeckA.handCapacity - selectedDeckB.handCapacity,
      winRateDelta: Number(((selectedDeckA.winRate - selectedDeckB.winRate) * 100).toFixed(1)),
      hpLossDelta: Number((selectedDeckA.avgHealthLost - selectedDeckB.avgHealthLost).toFixed(1)),
      sanityDelta: Number((selectedDeckA.avgSanityExpended - selectedDeckB.avgSanityExpended).toFixed(1)),
    };
  ...
  const diffCardBreakdown = useMemo(() => {
    ...
    const bMap = new Map(selectedDeckB.cards.map((c) => [c.name, c]));
    const aMap = new Map(selectedDeckA.cards.map((c) => [c.name, c]));

    const sharedCards: Array<{ id: string; name: string; copiesA: number; copiesB: number; category: CardCategory }> = [];
    ...
    for (const aCard of selectedDeckA.cards) {
    ...
    for (const bCard of selectedDeckB.cards) {
    ...
    <div className="balance-matrix-dashboard" data-testid="balance-matrix-dashboard">
      ...
      <header className="balance-dashboard-header">
        ...
        <div className="balance-sub-nav">
          ...
          >
            <BarChart3 size={16} />
            <span>卡牌/遺物數值天梯 (生心理散布圖與曲線)</span>
          </button>
          <button
            type="button"
            className={`balance-nav-btn ${subTab === 'topology' ? 'active' : ''}`}
      ...
      {/* ADR-0038: 自然流派拓撲生態 (300~500套牌 MDS 星系散布圖與 Diff) */}
      {subTab === 'topology' && (
          ...
          <div className="topology-controls-bar">
            <div className="topology-filter-group">
              <div className="topology-select-wrap">
                ...
                >
                  <option value="all">全部自然湧現流派 ({allDeckNodes.length} 套牌)</option>
              ...
              <div className="topology-select-wrap">
                <span style={{ color: '#94a3b8' }}>
                  共顯示 <strong style={{ color: '#ffd700' }}>{filteredDeckNodes.length}</strong> 套代表性牌組
                </span>
              </div>
            </div>
              ...
              {isDiffMode && selectedDeckA && selectedDeckB ? (
                ...
                <div className="diff-comparison-view" data-testid="diff-comparison-view">
                  <div className="diff-header-row">
                    <div>
                      <span className="diff-badge diff-badge-a">牌組 A</span>
                  ...
                  <div className="diff-header-row">
                    <div>
                      <span className="diff-badge diff-badge-b">牌組 B</span>
                      <strong style={{ marginLeft: 6, fontSize: 13, color: '#f8fafc' }}>
                        {selectedDeckB.name}
                      </strong>
                    </div>
                  ...
                  <table className="diff-stats-table">
                    <thead>
                      <tr>
                        <th>評測指標</th>
                        <th style={{ color: '#38bdf8' }}>牌組 A</th>
                        <th style={{ color: '#f59e0b' }}>牌組 B</th>
                        <th>差值 (A - B)</th>
                      </tr>
                    </thead>
                    <tbody>
                      <tr>
                        <td>綜合評分</td>
                        <td>{selectedDeckA.overallScore} 分</td>
                        <td>{selectedDeckB.overallScore} 分</td>
                        <td style={{ color: diffStats && diffStats.scoreDelta >= 0 ? '#10b981' : '#ef4444', fontWeight: 700 }}>
                          {diffStats && diffStats.scoreDelta > 0 ? `+${diffStats.scoreDelta}` : diffStats?.scoreDelta} 分
                        </td>
                      </tr>
                      <tr>
                        <td>敵怪對弈勝率</td>
                        <td>{(selectedDeckA.winRate * 100).toFixed(1)}%</td>
                        <td>{(selectedDeckB.winRate * 100).toFixed(1)}%</td>
                        <td style={{ color: diffStats && diffStats.winRateDelta >= 0 ? '#10b981' : '#ef4444' }}>
                          {diffStats && diffStats.winRateDelta > 0 ? `+${diffStats.winRateDelta}` : diffStats?.winRateDelta}%
                        </td>
                      </tr>
                      <tr>
                        <td>平均生命損失</td>
                        <td>{selectedDeckA.avgHealthLost} 點生命</td>
                        <td>{selectedDeckB.avgHealthLost} 點生命</td>
                        <td style={{ color: diffStats && diffStats.hpLossDelta <= 0 ? '#10b981' : '#ef4444' }}>
                          {diffStats && diffStats.hpLossDelta > 0 ? `+${diffStats.hpLossDelta}` : diffStats?.hpLossDelta} 點生命
                        </td>
                      </tr>
                      <tr>
                        <td>平均心智消耗</td>
                        <td>{selectedDeckA.avgSanityExpended} 點理智</td>
                        <td>{selectedDeckB.avgSanityExpended} 點理智</td>
                        <td style={{ color: diffStats && diffStats.sanityDelta <= 0 ? '#10b981' : '#ef4444' }}>
                          {diffStats && diffStats.sanityDelta > 0 ? `+${diffStats.sanityDelta}` : diffStats?.sanityDelta} 點理智
                        </td>
                      </tr>
                      <tr>
                        <td>牌庫總數</td>
                        <td>{selectedDeckA.totalCards} 張</td>
                        <td>{selectedDeckB.totalCards} 張</td>
                        <td>{diffStats && diffStats.cardsDelta > 0 ? `+${diffStats.cardsDelta}` : diffStats?.cardsDelta} 張</td>
                      </tr>
                      <tr>
                        <td>初始手牌容量</td>
                        <td>{selectedDeckA.handCapacity} 張</td>
                        <td>{selectedDeckB.handCapacity} 張</td>
                        <td>{diffStats && diffStats.handDelta > 0 ? `+${diffStats.handDelta}` : diffStats?.handDelta} 張</td>
                      </tr>
                    </tbody>
                  </table>
                    ...
                    <div>
                      ...
                      <div style={{ marginTop: 8 }}>
                        <span style={{ fontSize: 11, color: '#38bdf8' }}>
                          牌組 A 獨有卡牌 ({diffCardBreakdown.aOnlyCards.length} 種)：
                        </span>
                      ...
                      <div style={{ marginTop: 8 }}>
                        <span style={{ fontSize: 11, color: '#f59e0b' }}>
                          牌組 B 獨有卡牌 ({diffCardBreakdown.bOnlyCards.length} 種)：
                        </span>
                        <div className="diff-card-pills-container">
                ...
                /* Single Deck Inspector (or Diff Mode Waiting for 2nd Deck) */
                selectedDeckA && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                    {isDiffMode && !selectedDeckB && (
                      <div className="diff-prompt-banner">
                        已選定牌組 A【{selectedDeckA.name}】。請在左側星系散布圖點選第二套牌組以啟動並列 Diff 深度比對！
                      </div>
                    )}

                    <div>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
                        <h4 style={{ margin: 0, fontSize: 15, color: '#ffd700' }}>{selectedDeckA.name}</h4>
                        <span
                          style={{
                            fontSize: 11,
                            padding: '2px 8px',
                    ...
                    <div className="deck-metrics-grid">
                      <div className="deck-metric-box">
                        ...
                        <span className="deck-metric-val" style={{ color: getHeatmapColor(selectedDeckA.overallScore) }}>
                          {selectedDeckA.overallScore} 分
                        </span>
                      </div>
                      <div className="deck-metric-box">
                        <span className="deck-metric-lbl">對弈勝率</span>
                        <span className="deck-metric-val">{(selectedDeckA.winRate * 100).toFixed(1)}%</span>
                      </div>
                      <div className="deck-metric-box">
                        <span className="deck-metric-lbl">平均生命損失</span>
                        <span className="deck-metric-val">{selectedDeckA.avgHealthLost} 點生命</span>
                      </div>
                      <div className="deck-metric-box">
                        <span className="deck-metric-lbl">平均心智消耗</span>
                        <span className="deck-metric-val">{selectedDeckA.avgSanityExpended} 點理智</span>
                      </div>
                      <div className="deck-metric-box">
                        <span className="deck-metric-lbl">牌庫張數</span>
                        <span className="deck-metric-val">{selectedDeckA.totalCards} 張</span>
                      </div>
                      <div className="deck-metric-box">
                        <span className="deck-metric-lbl">初始手牌容量</span>
                        <span className="deck-metric-val">{selectedDeckA.handCapacity} 張</span>
                      </div>
                    </div>

                    {/* Driving Combos */}
                    {selectedDeckA.drivingCombos.length > 0 && (
                      <div>
                        <span style={{ fontSize: 12, fontWeight: 600, color: '#f8fafc' }}>核心驅動連鎖：</span>
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 4 }}>
                          {selectedDeckA.drivingCombos.map((combo, idx) => (
                            <span
                              key={idx}
                              style={{
                                fontSize: 11,
                    ...
                    <div>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                        <span style={{ fontSize: 12, fontWeight: 600, color: '#f8fafc' }}>
                          牌庫卡表明細 ({selectedDeckA.cards.length} 種 / 共 {selectedDeckA.totalCards} 張)：
                        </span>
                      </div>
                      <div className="deck-cards-list">
                        {selectedDeckA.cards.map((c) => {
