...
export const TIER_1_CARDS: Card[] = getCardsByIds([
  ...
  'reward_double_tap',
  'reward_veteran_instinct',
  'reward_ancient_amulet_1',
  'reward_void_fire_1',
  'reward_dread_whisper_1',
  'reward_whispers_of_madness',
  'reward_astral_insight_1',
  'reward_astral_refraction',
  'reward_first_aid_1',
  'reward_field_bandage',
  'reward_tactical_feint',
 ...
 * - 若傳入 occupationId，嚴格過濾僅產出該職業專屬卡與中立通用卡
 */
export function generateRewardCardsForDepth(
  ...
  const targetCount = count ?? (isBoss && depth === 2 ? Math.min(4, pool.length) : Math.min(3, pool.length));
