...
describe('Card Mechanics SVD Embedding & Cosine Similarity Matrix (Issue #68)', () => {
  ...
  it('incorporates 35 enemy counter fingerprints into feature vectors and SVD embeddings (Issue #68)', () => {
    const card = compendiumCards[0];
    const baseFeatures = extractCardMechanicsFeatures(card);

    // Mock 35 enemy counter fingerprints (win rates against 35 enemies)
    const mockFingerprints = new Map<string, number[]>();
    for (const c of compendiumCards) {
      mockFingerprints.set(c.id, Array.from({ length: 35 }, (_, idx) => 0.4 + (idx % 5) * 0.1));
    }

    const extendedFeatures = extractCardMechanicsFeatures(card, undefined, mockFingerprints);
    expect(extendedFeatures).toHaveLength(baseFeatures.length + 35);
